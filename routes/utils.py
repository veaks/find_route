...
def dfs_path(graph, start, goal):
    """Функция посика всех возможных маршрутов
    из одного города в другой. Вариант посещения
    одного о того же города более одного раза
    не рассматривается.
    """
    stack = [(start, [start])]
    while stack:
...
def get_routes(request, form) -> dict:
    ...
    all_trains = {}
    for q in qs:
        all_trains.setdefault((q.from_sity_id, q.to_city_id), [])
        all_trains[(q.from_sity_id, q.to_city_id)].append(q)
